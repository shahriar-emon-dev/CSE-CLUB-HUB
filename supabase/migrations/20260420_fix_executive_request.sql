...
begin
  if auth.uid() = old.id then
    ...
    if new.role_request is distinct from old.role_request then
      ...
      raise exception 'Invalid role request mutation. Use dedicated request and withdrawal functions.' using errcode = '42501';
    end if;
  ...
  if not public.is_admin(auth.uid()) then
    raise exception 'Only admins can update privileged profile fields.' using errcode = '42501';
  end if;

  if new.role is distinct from old.role and new.role not in ('student', 'executive', 'admin') then
    raise exception 'Invalid role value.' using errcode = '22P02';
  end if;
...
begin
  ...
  if not found then
    raise exception 'Request not allowed for current account state.';
  end if;
end;
$$;
