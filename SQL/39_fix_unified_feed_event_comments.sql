-- ============================================================================
-- Migration: 39_fix_unified_feed_event_comments.sql
-- Description: Updates the unified_feed_timeline view to count event comments dynamically
-- ============================================================================
...
SELECT
    ...
    e.created_at,
    e.updated_at,
    (SELECT count(*)::bigint FROM public.comments cmt WHERE cmt.entity_id = e.id AND cmt.entity_type = 'event' AND cmt.is_deleted = false) as comment_count,
    0::bigint AS favorite_count,
    0::bigint AS fire_count,
    0::bigint AS hand_count,
    e.event_date,
    e.end_date,
    e.venue,
    e.category,
